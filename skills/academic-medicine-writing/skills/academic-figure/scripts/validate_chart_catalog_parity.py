#!/usr/bin/env python3
"""Validate that medical R and Python chart catalogs expose the same families."""
...
def compare_catalogs(root: Path) -> tuple[list[str], list[str]]:
    ...
    r_families = extract_display_families(r_catalog)
    python_families = extract_display_families(python_catalog)
    return r_families, python_families
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="academic-figure skill root containing references/",
    )
